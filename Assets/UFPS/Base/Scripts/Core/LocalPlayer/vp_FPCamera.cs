...
public class vp_FPCamera : vp_Component
{
	...
	protected virtual void UpdateMouseLook()
	{
		...
		//f make sure the defined smoothing vars are within range
	...
	protected virtual void UpdateJoystickLook()
	{
		// --- fetch mouse input ---
		
		m_MouseMove.x = state.ThumbSticks.Right.X * Time.timeScale;
		m_MouseMove.y = state.ThumbSticks.Right.Y * Time.timeScale;
