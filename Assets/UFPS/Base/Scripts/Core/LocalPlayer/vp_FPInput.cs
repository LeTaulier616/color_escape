...
public class vp_FPInput : MonoBehaviour
{
	...
	protected virtual void Update()
	{
		...
		state = GamePad.GetState(playerIndex, (GamePadDeadZone)0.24f);
	...
	protected virtual void InputMove()
	{
		...
		{
			Player.InputMoveVector.Set(new Vector2(state.ThumbSticks.Left.X, state.ThumbSticks.Left.Y));
		}
	...
	{
		if(OnButtonDown(state.Buttons.LeftStick, prevState.Buttons.LeftStick))
		{
			Player.Run.TryStart();
		}

		if(Player.Run.Active && state.ThumbSticks.Left.X == 0.0f && state.ThumbSticks.Left.Y == 0.0f)
		{
			Player.Run.TryStop();
		}
	...
	protected virtual void InputJump()
	{
		...
		if (vp_Input.GetButton("Jump") || state.Buttons.A == ButtonState.Pressed)
	...
	protected virtual void InputCrouch()
	{
		...
		if (vp_Input.GetButton("Crouch") || state.Buttons.B == ButtonState.Pressed)
